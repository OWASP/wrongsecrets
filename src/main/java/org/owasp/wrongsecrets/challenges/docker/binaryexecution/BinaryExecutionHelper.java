...
public class BinaryExecutionHelper {
  ...
  public String executeGoCommand(String guess) {
    try {
      ...
      if (Strings.isNullOrEmpty(guess)) {
        result = executeCommand(execFile, BinaryOperation.Spoil, "");
      } else {
        result = executeCommand(execFile, BinaryOperation.Guess, guess);
      }
      log.info(
          "stdout challenge {}: {}",
          challengeNumber,
  ...
  public String executeCommand(String guess, String fileName) {
    ...
    if (Strings.isNullOrEmpty(guess)) {
      binaryOperation = BinaryOperation.Spoil;
    } else {
      binaryOperation = BinaryOperation.Guess;
    }
    try {
      File execFile = createTempExecutable(fileName);
      String result = executeCommand(execFile, binaryOperation, guess);
      deleteFile(execFile);
      log.info(
          "stdout challenge {}: {}",
          challengeNumber,
  ...
  private String executeCommand(File execFile, BinaryOperation binaryOperation, String guess)
    ...
    if (binaryOperation.equals(BinaryOperation.Spoil)) {
