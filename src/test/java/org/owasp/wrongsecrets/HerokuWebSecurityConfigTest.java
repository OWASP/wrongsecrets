...
class HerokuWebSecurityConfigTest {
  ...
  void shouldRedirectWhenProtoProvided() {
    var restTemplate = builder.defaultHeader("x-forwarded-proto", "value").build();
    var rootAddress =
        "http://localhost:"
            + port
            + "/heroku"; // note we loosely ask for "heroku" to be part of the url

    // in Spring security 2022 we no longer can bind to the new port of the redirect as that is not
    // preset. hence this exception proves the redirect
    Exception exception =
        assertThrows(
