...
class ThymeleafToStaticConverter:
    ...
    def generate_mock_challenges(self):
        ...
        difficulties = ["⭐", "⭐⭐", "⭐⭐⭐", "⭐⭐⭐⭐", "⭐⭐⭐⭐⭐"]
        techs = ["DEVOPS", "GIT", "FRONTEND", "DEVOPS", "AWS", "AZURE", "DOCKER", "DOCKER", "AWS", "AWS"]
        environments = ["Docker", "Docker", "Docker", "Docker", "AWS", "Azure", "Docker", "Docker", "AWS", "AWS"]
    ...
    def process_thymeleaf_syntax(self, content, template_name):
        ...
        content = re.sub(r'<html[^>]*layout:decorate="~\{[^}]+\}"[^>]*>', '<html lang="en">', content)
        content = re.sub(r'xmlns:layout="[^"]*"', '', content)
        content = re.sub(r'xmlns:th="[^"]*"', '', content)
        
        # Replace layout fragments
        content = re.sub(r'<div[^>]*layout:fragment="content"[^>]*>', '<div class="container">', content)
    ...
    def replace_th_each(self, content):
        ...
        if 'th:each="canarytokenURL : ${canarytokenURLs}"' in content:
            ...
            content = re.sub(pattern, f'<ul style="word-wrap: break-word">{canary_items}</ul>', content, flags=re.DOTALL)
    ...
    def replace_th_href(self, content):
        ...
        content = re.sub(r'th:href="\$\{swaggerURI\}"', f'href="{self.mock_data["swaggerURI"]}"', content)
        return content
    ...
    <div class="preview-banner">
        ...
        <small>This is a static preview of PR #{self.pr_number}. Some dynamic content may be simplified or use mock data.</small>
            ...
            if '<div class="container">' in content:
                content = content.replace('<div class="container">', f'<div class="container">{banner}')
            elif '<body>' in content:
                content = content.replace('<body>', f'<body><div class="container">{banner}</div>')
    ...
    def generate_challenge_page(self):
        ...
        content = re.sub(r'th:text="\$\{challenge\.name\}"', f'>{mock_challenge["name"]}<', content)
        content = re.sub(r'th:text="\$\{challenge\.stars\}"', f'>{mock_challenge["stars"]}<', content)
        content = re.sub(r'th:text="\$\{challenge\.tech\}"', f'>{mock_challenge["tech"]}<', content)
