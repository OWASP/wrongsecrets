...
PS3='Select ingress or port forward for access: '
options=("Port forward to localhost" "GCP loadbalancer ingress" "Quit")
select opt in "${options[@]}"
do
    case $opt in
        "Port forward to localhost")
            echo "you chose choice 1"
            kubectl port-forward $(kubectl get pod -l app=secret-challenge -o jsonpath="{.items[0].metadata.name}") 8080:8080
            echo "Run terraform destroy to clean everything up" 
            break
			;;
        "GCP loadbalancer ingress")
            echo "Deploying ingress"
            kubectl apply -f k8s/k8s-gke-ingress.yaml
            while [[ -z $(kubectl get ingress basic-ingress --output jsonpath='{.status.loadBalancer.ingress[].ip}') ]]; do echo "waiting for ingress this will take a few minutes... last check: $(date +\%T)" && sleep 10; done
            echo " "
