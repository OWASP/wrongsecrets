        ...
        if '[bot]' not in username:
            if ranking >= 100:
                top_contributors.append(
                    {'username': username, 'ranking': ranking})
            else:
...
def get_contibutor_list(project: str, user_token: str) -> list:
    headers = {'X-GitHub-Api-Version': '2022-11-28',
               'Accept': 'application/vnd.github+json', 'Authorization': 'Bearer ' + user_token}
    r = requests.get('https://api.github.com/repos/OWASP/' +
                     project+'/contributors', headers=headers)
...
if token != None:
    ...
    wrongsecrets_binaries_list = get_contibutor_list(
        'wrongsecrets-binaries', token)
    print_list(wrongsecrets_binaries_list[0],
               "Wrong Secrets Binaries Top contributors:")
    print_list(wrongsecrets_binaries_list[1], "\nContributors:")

    # Prints the Wrong Secrets CTF contributors list
    wrongsecrets_ctf_list = get_contibutor_list(
        'wrongsecrets-ctf-party', token)
